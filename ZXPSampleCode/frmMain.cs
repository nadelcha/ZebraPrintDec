...
namespace ZXPSampleCode
{
    public partial class frmMain : Form
    {
        ...
        private string[] _lbSamples = { "Print EDEC Evidence NFC",
                                        "Print 1",      // image & shapes
        ...
        private void InitializePrintTestSelections(string test)
        {
            try
            {
                ...
                {
                    case "Print EDEC Evidence NFC":
                        this.tbDescr.Text = "Print EDEC Evidence Card:\r\n" +
                            "   Prints a standard EDEC Evidence Card with logo and \r\n" +
                            "   an example EDEC bag serial number and QR";
                        break;
                    case "Print 1":
        ...
        private void PerformPrintTest(ref ZXPSampleCode zxp, string test, string cardType)
        {
            try
            {
                ...
                {
                    case "Print EDEC Evidence NFC":
                        int start = Decimal.ToInt32(snStart.Value);
                        int end = Decimal.ToInt32(snEnd.Value);
                        string prefix = tbPrefix.Text;
                        int digits = Decimal.ToInt32(snDigits.Value);
                        //for(int i = start; i <= end; i++) {
                        //    string status = string.Empty;
                        //    string serialNumber = getSerialString(prefix, i, digits);
                        //    lbJobStatus.Text = "Printing card " + serialNumber;
                        //    int actionID = 0;
                        //    Job job = new Job();

                        //    actionID = zxp.Print_Edec_Evi_Card(cardType, serialNumber, ref job);
                        //    zxp.JobWait(ref job, actionID, 180, out status);
                        //    zxp.Disconnect(ref job);
                        //    lbJobStatus.Text = "Job Result = " + zxp.Msg;
                        //    Refresh();
                        //}
                        zxp.Print_Edec_Evi_Card(cardType,prefix, start, end, digits);
                        break;
                    case "Print 1":
        ...
        private void snStart_ValueChanged(object sender, EventArgs e)
        {
            setSnPreviews();
        }

        private void setSnPreviews()
        {
            int start = Decimal.ToInt32(snStart.Value);
            int end = Decimal.ToInt32(snEnd.Value);
            string prefix = tbPrefix.Text;
            ...
            snPrevStart.Text = getSerialString(prefix, start, digits);
            snPrevEnd.Text = getSerialString(prefix, end, digits);
        }
        ...
        private void snEnd_ValueChanged(object sender, EventArgs e)
        {
            setSnPreviews();
        }

        private void snDigits_ValueChanged(object sender, EventArgs e)
        {
            setSnPreviews();
        }
